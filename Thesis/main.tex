...
I realised that the code wasn't very concise and did not seem to adhere to current standards of pytorch programs. 
...
Regardless of the fact that I did not have access to such a machine I considered this bad practice and decided to rewrite the dataloader from scratch. 
...
This way I could simply adapt a voice activity detector sample already provided by Lhotse. This way I did not have to work with the raw transcriptions in the supervision segments whose additional information isn't needed if I'm just working with binary labels. 
...
\subsection{Method}
In the experiments shown we first selected the same amount of laughter and non-laughter (1-to-1).
All samples were one second long and during training no subsampling was applied. 
The non-laughter segments were selected on a random basis.
