...
This original goal of the project was to create a filter for laughter and applause and integrate it as a new \texttt{filtered} option in a video call system. This goal was not achieved, but the project provides the foundation for the laughter recognition component of such a system.
Even though the current performance is not sufficient, the insights about the ICSI corpus, the outlined process of training such a model, and the published code \citep{Wolter_A_Machine_Learning_2022} will benefit future work.
...
It should be noted, however, that some parts of the code, especially \verb|train.py|, still contain large parts of the code by \citet{gillick2021robust} that I criticised as cumbersome and not adhering to common PyTorch standards. 
...
Following prior research and disregarding the non-transcribed regions completely is not a solution for our use case. Using continues audio tracks instead of disconnected laughter and non-laughter samples, better resembles the use case of laughter detection as part of a video call system. 
...
Future research can also use the published code \citep{Wolter_A_Machine_Learning_2022} to continue the experiments from \autoref{cha:experiments}. Conducting a grid search on the variable parameters in those experiments will determine the corpus subset that is most representative of the whole ICSI corpus. Adapting the feature representation will be another dimension to experiment with. The thesis mentions three ideas for this: different spectrogram dimensions, feature augmentation, and subsampling. 

Making the model work in real-time is another important part for the continuation of this project. \autoref{sec:real-time} provided some theoretical considerations as well as a baseline for future work.
Lastly, ethical considerations like user privacy and emotional harm were only covered superficially in \autoref{cha:practicality}. For a system that continually processes audio from end-users a more thorough ethical review is vital.
