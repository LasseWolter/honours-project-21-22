...
To allow for comparisons, I used the same ResNet architecture as \citet{gillick2021robust}.
...
The ratio of non-laughter subclasses (\ref{table:segment-types}) was not enforced. This characteristic is investigated in \autoref{sec:exp2}.
...
\subsection{Method}
To select non-laughter segments across a variety of meetings \verb|create_data_df| uses the following selection process. The sampled channel within that meeting is chosen at random.
...
The precision-recall curve does not give much insight into this, as all models show a similar performance. Even though we see a difference in the plotted data points, e.g. the model trained on the 1-to-20 training data has more data points in the area of $\mathtt{precision} >70\%$.
...
More data points in the area of $\mathtt{precision} > 70\%$ do not imply a higher precision. Data points in this region could be achieved with the other models by adjusting their thresholds accordingly.
However, we conclude that the models trained on the ICSI dataset perform significantly better than the model trained on the Switchboard dataset, which is the expected behaviour.

Another inisght is deduced from the confusion matrices. Whereas the baseline has an even split between speech and silence, the three newly trained models rarely misclassify speech. 
...
\subsection{Analysis}
The goal of these experiments was to analyse the impact of diversity within the non-laughter segments. Using only a few silence segments decreases model performance significantly. The impact of slightly changing amounts of noise, speech and laughter but keeping the number of silence segments constant yielded a small performance impact. 
