...
\hspace*{-1cm}                                                           
\subfloat[baseline]{\includegraphics[width = 3.5in]{imgs/conf_matrix/init_eval_train.png}}\\ 
\end{tabular}
...
The precision recall curve does not give much insight into this, as all models show a similar performance. Even though we see a difference in the plotted data points, e.g. the model trained on the 1-to-20 training data has a more data points in the are of precision >70\%.
This difference needs to be seen in context. Thresholds are a post-processing step that do not actually affect the model output itself. 
Having more data points in the area of $precision > 70\%$ does not mean that the model has a higher precision. Data points in this region could be achieved with the other models by adjusting their thresholds accordingly.
Other than that we can only conclude that the models trained the ICSI dataset perform significantly better than the model trained on the Switchboard dataset, which is the expected behaviour.

The confusion matrices give us an interesting insight. Whereas the baseline has an even split between speech and silence, the three newly trained model rarely misclassify speech. The main source of misclassification are the non-transcribed regions. This brings up the same question I raised during the initial evaluation. Why is the model not able to confidently separate laughter from silence?
...
Thus, number of non-laughter segments was fixed to 20. 20 was chosen to maximise the amount of non-laughter segments and be able to use the 1-to-20 results from \auotref{sec:exp-1} as a baseline.
...
\hspace*{-1cm}                                                           
\subfloat[baseline]{\includegraphics[width = 3.5in]{imgs/conf_matrix/random/1_to_20_random.png}}\\ 
\end{tabular}
\caption{Confusion matrices for different partitioning of non-laughter types:  \autoref{tab:exp-2-subclass-split}}
\label{fig:conf-matrix-struc}
\end{figure}


% \textbf{General ML advice}
% When using online validation, the number of validation batches matters a lot. To get a better intuition about this I plotted the metrics during training for different batch sizes and log frequency

\section{Investigating large amount of retrieved silence segments}
Both, the initial evaluation (\autoref{cha:model-evaluation}) and the experiments(\autoref{cha:experiments}) raise questions about the silence segments in the corpus. Thus, I decided to manually investigate some audio tracks, to see if I can spot anything unusual. This is a crude analysis with the purpose of coming up with a hypothesis for the large amount retrieved silence segments.
I randomly selected 5 from the 468 audio tracks:
\begin{enumerate}
    \item Bmr027: chan5
    \item Bdb001: chan4
    \item Btr001: chan7
...
Assuming splitting this window in the middle, similar to \auotref{fig:know_window}, this meant around 0.5s before and 0.5s after the considered frame. This is an unacceptable latency.
