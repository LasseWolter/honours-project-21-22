...
This training performance, measured on a GPU machine, was significantly worse than the evaluating the pre-trained model from \autoref{cha:model-evaluation} on a CPU-only machine. 
...
\autoref{fig:conf-matrix-rand} shows a confusion matrices for different thresholds to deepen the understanding of the misclassified laughter samples. Note that the baseline is not same confusion matrix as \autoref{fig:initial-conf-matrix} because this time we only evaluate on the development set.
